...
if(!empty($data->telefone) && !empty($data->email)){
    ...
    if($contato->alterarContato()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Contato do usuário atualizada com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possivel atualizar o contato."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para alterar o contato"));
}
