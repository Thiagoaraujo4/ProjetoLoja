...
if(!empty($data->id_pedido) && !empty($data->id_produto) && !empty($data->quantidade)){
    ...
    if($detalhepedido->cadastro()){
        ...
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não autorizado o cadastro do detalhepedido. "));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para cadastrar o detalhepedido"));
}
