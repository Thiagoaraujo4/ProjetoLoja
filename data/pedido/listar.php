<?php
/* Este cabeçalho permite acesso a listagem de pedido 
...
/*
 O arquivo pedido.php foi incluido para que a classe pedido fosse utilizada. vale 
lembrar que esta classe possui o CRUD para o pedido.
 */
...
/*
Instância da classe Usuário e, portanto, criação do objeto chamado $pedido.
Isso fará com que todas as funções que estão dentro da classe Uauraio sejam transferidas
para o obejto $pedido.
...
Dentro da função listar() temos uma consulta no formato sql que seleciona todos os usuário("Select * from pedido")
*/
$stmt = $pedido->listar();

/*
Se a consulta retornar uma quantidade maior que 0(zero), então será construido um array com os dados dos 
usuários. Caso contrário será exibido uma mensagem que não pedidos cadastrados.
*/

if($stmt->rowCount() > 0){

    /*
    Para organizar os pedidos cadastrados em banco e exibi-los em tela, foi criado uma array com o nome de
    saida e assim guardar todos pedidos.
    */
    ...
    Assim será mais fácil de adicionar no array de pedidos para ser apresentado ao pedido.
    */
    while($linha = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        */
        ...
        extract($linha);
        /*
        Pegar um campo por vez do comando extract e adicionar em um array de itens, pois será assim que os pedidos
        serão  tratados, um pedido por vez com seus respectivos dados.
        */
        ...
        ao pedido_arr[saida] um item que é um pedido com seus respectivos dados.
        */
...
/*
Pegamos o array pedido_arr que foi construido em php com os dados dos pedidos
e convertemos para o formato json para exibir ao pedido requisitante.
*/
echo json_encode($pedido_arr);
    ...
    /*
    O comando header(cabeçalho) responde os pedido o status code 400(badrequest)
    ...
    */
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Não há pedido cadastrados"));
