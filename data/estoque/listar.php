<?php
/* Este cabeçalho permite acesso a listagem de estoque 
...
/*
 O arquivo estoque.php foi incluido para que a classe estoque fosse utilizada. vale 
lembrar que esta classe possui o CRUD para o estoque.
 */
...
/*
Instância da classe Usuário e, portanto, criação do objeto chamado $estoque.
Isso fará com que todas as funções que estão dentro da classe Uauraio sejam transferidas
para o obejto $estoque.
...
Dentro da função listar() temos uma consulta no formato sql que seleciona todos os usuário("Select * from estoque")
*/
$stmt = $estoque->listar();

/*
Se a consulta retornar uma quantidade maior que 0(zero), então será construido um array com os dados dos 
usuários. Caso contrário será exibido uma mensagem que não estoques cadastrados.
*/

if($stmt->rowCount() > 0){

    /*
    Para organizar os estoques cadastrados em banco e exibi-los em tela, foi criado uma array com o nome de
    saida e assim guardar todos estoques.
    */
    ...
    Assim será mais fácil de adicionar no array de estoques para ser apresentado ao estoque.
    */
    while($linha = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        */
        ...
        extract($linha);
        /*
        Pegar um campo por vez do comando extract e adicionar em um array de itens, pois será assim que os estoques
        serão  tratados, um estoque por vez com seus respectivos dados.
        */
        ...
        ao estoque_arr[saida] um item que é um estoque com seus respectivos dados.
        */
...
/*
Pegamos o array estoque_arr que foi construido em php com os dados dos estoques
e convertemos para o formato json para exibir ao estoque requisitante.
*/
echo json_encode($estoque_arr);
    ...
    /*
    O comando header(cabeçalho) responde os estoque o status code 400(badrequest)
    ...
    */
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Não há estoque cadastrados"));
